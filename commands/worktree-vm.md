...
Drive `worktree-vm` for the **current git worktree** following the `worktree-vm`
skill. The requested action is: `$ARGUMENTS` (default to `up` if empty).

Rules:
- Resolve the CLI as `worktree-vm` on PATH, else this plugin's `bin/worktree-vm`.
- For `up` (or empty): follow the skill's LONG-RUNNING procedure — run it in the
  background and emit a ~60s heartbeat with `worktree-vm status` until the health
...
- For `test`: run `worktree-vm test $ARGUMENTS-after-test` inside the VM and report
